...
public class LineAnalysis implements tfvisConstants{
	 ...
	 * @param pro     プログラム全体のデータクラス
	 * @param m_Token クラスのトークンリスト(要)
	 * @param m_Line  行データを入れるリスト(空)
	 */
	static public void exe(Program pro,ArrayList<Token> m_Token,ArrayList<Line> m_Line){
		...
		for(int k=0;k<m_Token.size();k++) {
			...
			//ループ終了
			if(nowID == For + End){m_Line.get(m_Line.size()-1).addEvent(Ev_ForEnd);continue;}
			
			//イベント登録
			if(nowID == Return){nowLine.addEvent(Ev_Return);}
			if(nowID == For){nowLine.addEvent(Ev_For);}
			
			if(nowID == If){	
				nowLine.addEvent(Ev_Route);
			}
			
			if(nowID == Else + End){continue;}
			if(nowID == If + End){m_Line.get(m_Line.size()-1).addEvent(Ev_IfEnd);continue;}
			if(nowID == Branch + End){/*m_Line.get(m_Line.size()-1).addEvent(Ev_BranchEnd)*/;continue;}
			
			if(nowID == While){nowLine.addEvent(Ev_While);}
			if(nowID == While + End){m_Line.get(m_Line.size()-1).addEvent(Ev_WhileEnd);continue;}	
			
			if(nowID == Try){nowLine.addEvent(Ev_Try);}
			if(nowID == Try + End){m_Line.get(m_Line.size()-1).addEvent(Ev_TryEnd);continue;}
			if(nowID == Catch){nowLine.addEvent(Ev_Catch);}
			if(nowID == Catch + End){m_Line.get(m_Line.size()-1).addEvent(Ev_CatchEnd);continue;}
			if(nowID == Finally){nowLine.addEvent(Ev_Finally);}
			if(nowID == Finally + End){m_Line.get(m_Line.size()-1).addEvent(Ev_FinallyEnd);continue;}
			
			if(nowID == Switch){nowLine.addEvent(Ev_Switch);}
			if(nowID == Switch + End){m_Line.get(m_Line.size()-1).addEvent(Ev_SwitchEnd);continue;}
			if(nowID == Case){nowLine.addEvent(Ev_Case);}
			if(nowID == Case + End){continue;}
			if(nowID == Default){nowLine.addEvent(Ev_Case);}
			if(nowID == Default + End){continue;}
			
			if(nowID == ObjectExp){nowLine.addEvent(Object);}
			
			if(nowID == Do){nowLine.addEvent(Ev_Do);}
			...
			if(k<m_Token.size()-1 && m_Token.get(k+1).m_State.equals("{") == true && !m_Token.get(k).m_State.equals("=")){
				...
				changeLine=true;
			}else if(state.equals("}") == true && (nowID == Method + End || nowID == Constructor+End)){
					indent--;
					lineHead=true;
				...
				if(lineHead){nowLine.setIndent(indent);;lineHead=false;}
				
				//*extends //*の場所に空白
				if(m_Token.get(k).m_State.equals("extends")){nowLine.addCode(" ");}
				
				nowLine.addCode(state);
				
				if(Tools.checkNeedSpace(m_Token,k)){nowLine.addCode(" ");}
	...
	static public void eventCheck_CallMethod(Program pro,Token token,Line line){
		...
		if(token.m_ID == CoiType1 + Identifier){return;}
		
		if(line.checkHasEvent(Ev_MethodStart)==true){return;}
