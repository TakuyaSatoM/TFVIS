...
public class Tools implements tfvisConstants {
	...
	static public String getVarName(ArrayList<Token> tokens, int start) {
		...
		if (tokens.get(now).m_State.equals("[") == false) {
			return out;
		}
		out += "[";

		for (int i = now + 1;; i++) {
			if (tokens.get(i).m_State.equals("]")) {
				break;
			}
	...
	static public boolean checkNeedSpace(ArrayList<Token> tokens, int now) {
		...
		if (now > 1) {
			...
			if (Tools.getHasElement(tokens.get(now - 1).m_ID, Lbracket)
					&& Tools.getHasElement(tokens.get(now).m_ID, Rbracket)) {
				needSpace = true;
			}
			// Ary*ary;*の場所に空白
			if (tokens.get(now).m_ID == CoiType1 + Identifier && !tokens.get(now + 1).m_State.equals("[")) {
				needSpace = true;
			}
		}

		// final*int a; //*の場所に空白
		if (LocalVariable + Final == nowID) {
			needSpace = true;
		}

		// return*0; //*の場所に空白
		if (tokens.get(now).m_State.equals("return")) {
			needSpace = true;
		}

		// Ary ary=new*tmp; //*の場所に空白
		if (tokens.get(now).m_State.equals("new")) {
			needSpace = true;
		}

		// public*class //*の場所に空白
		if (tokens.get(now).m_State.equals("public")) {
			needSpace = true;
		}
		// protected*class //*の場所に空白
		if (tokens.get(now).m_State.equals("protected")) {
			needSpace = true;
		}
		// private*class //*の場所に空白
		if (tokens.get(now).m_State.equals("private")) {
			needSpace = true;
		}

		// class*tmp //*の場所に空白
		if (tokens.get(now).m_State.equals("class")) {
			needSpace = true;
		}

		// static*int //*の場所に空白
		if (tokens.get(now).m_State.equals("static")) {
			needSpace = true;
		}

		// case* //*の場所に空白
		if (tokens.get(now).m_State.equals("case")) {
			needSpace = true;
		}

		// else*if //*の場所に空白
		if (tokens.get(now).m_State.equals("else")) {
			needSpace = true;
		}

		// import* //*の場所に空白
		if (tokens.get(now).m_State.equals("import")) {
			needSpace = true;
		}

		// package* //*の場所に空白
		if (tokens.get(now).m_State.equals("package")) {
			needSpace = true;
		}

		// throws*IOExeption //*の場所に空白
		if (tokens.get(now).m_State.equals("throws")) {
			needSpace = true;
		}

		// extends* //*の場所に空白
		if (tokens.get(now).m_State.equals("extends")) {
			needSpace = true;
		}

		// implements* //*の場所に空白
		if (tokens.get(now).m_State.equals("implements")) {
			needSpace = true;
		}

		// final* //*の場所に空白
		if (tokens.get(now).m_State.equals("final")) {
			needSpace = true;
		}
		return needSpace;
	}
