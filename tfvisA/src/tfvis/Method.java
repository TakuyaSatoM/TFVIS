...
public class Method implements tfvisConstants{
		...
		for(Line line:lineList){
			...
			for(Integer event: line.getEventList()){
				...
				if(event==Ev_MethodEnd && beforeLine.checkHasEvent(Ev_Return)==false){//メソッド終了
					...
					if(line.getLoopInitTargetName().length()>0){
						...
						fout.println(indent+"TProbe.Input_LifeLimit(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+lineID+",\""+line.getLoopInitTargetName()+"\");");
						
						fout.print(line.getIndentText()+line.getCode().substring(0, line.getCode().length()-2) );
						fout.println(",TProbe.Input_Update(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+lineID+ ", \""+line.getLoopInitTargetName()+  "\","+line.getLoopInitTargetName()+",\""+line.getLoopInitTargetName()+"\","+inputState+")){");
						cancelCodeIn=true;
						insertedProbe=true;						
					}
					insertedProbe=true;			
					...
					isLoop = false;
					fout.println(indent+"TProbe.Input_LoopNext(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+lineID+ ","+loopStart.get(loopStart.size()-1)+");");
					fout.println(indent+"isLoop=false;");
					LoopEnd=lineID;
					insertedProbe=true;			
				}
				else if(event == Ev_Switch){
					fout.println(indent+"TProbe.Input_Switch(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+lineID+ ");");
					insertedProbe=true;
				}
				
				if(event == Ev_CatchEnd && !isLoop){
					fout.println(indent+"if(isLoop){");
					fout.println(indent+"TProbe.Input_LoopNext(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+CatchLoopEnd+ ","+CatchLoopEnd+");");				
					fout.println(indent+"TProbe.Input_LoopEnd(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+LoopEnd+ ");}");
					insertedProbe=true;		
				}
			}
			...
			for(Integer event: line.getEventList()){
				...
				if(line.checkHasEvent(Ev_For)==false){//for文の条件としての変数更新(カウンタ)は別処理(for)
					if(event==Ev_IntUpdate || event==Ev_DoubleUpdate || event==Ev_StringUpdate){//int型更新
						...
						String use="\"";
						{
							...
							if(line.getUseNum()==0){use="\"\"";}
							else{use+="\"";}
						}
						fout.println(indent+"TProbe.Input_Update(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+lineID+ ", "+ target+  ","+target2+","+use+","+inputState+");");
						insertedProbe=true;
						inputState = false;
					}
					if(event==Ev_IntUpdate+ArrayMode || event==Ev_DoubleUpdate+ArrayMode || event==Ev_StringUpdate+ArrayMode){//int型配列更新
						
						String target=line.getTarget();
						String use="";
						
						if(line.getUseNum() > 0){use=line.getUse(0);}
						
						fout.println(indent+"TProbe.Input_Update(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+lineID+ ", \""+ target+  "\","+target+","+inputState+");");
						insertedProbe=true;
						inputState = false;
					}					
				}
				...
				if(event==Ev_For && line.getLoopInitTargetName().length()>0){
					...
					fout.println(indent+"if(tvisLoopInit"+uniqueID+"==false){tvisLoopInit"+uniqueID+"=true;TProbe.Input_Update(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+lineID+",\""+line.getLoopInitTargetName()+"\","+line.getLoopInitTargetName()+",\"\","+inputState+");}");
					insertedProbe=true;			
					uniqueID++;
				}
				...
				if(event == Ev_ForEnd || event == Ev_WhileEnd || event == Ev_DoWhileEnd){//ループ終了
					isLoop = false;
					fout.println(indent+"TProbe.Input_LoopNext(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+loopStart.get(loopStart.size()-1)+ ","+loopStart.get(loopStart.size()-1)+");");
				...
				//プローブ[if条件成立
				if(event == Ev_Route){
					fout.println(indent+"TProbe.Input_Route(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+lineID+"  );");
					insertedProbe=true;		
				}
				
				//プローブ[Catch開始
				if(event == Ev_Catch){
					fout.println(indent+"TProbe.Input_Catch(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+lineID+"  );");
					insertedProbe=true;
				}
				if(event == Ev_Case){
					fout.println(indent+"TProbe.Input_Case(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+lineID+"  );");
					insertedProbe=true;
				}
	...
	void readProbe(Line line,int lineID,PrintWriter fout){
		...
		fout.println(line.getIndentText()+" "+"TProbe.Input_Read(TP_INSTANCEID,TP_METHODID,TP_METHODEXE,"+lineID+ ", "+use+"  );");
