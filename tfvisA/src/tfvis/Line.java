...
public class Line implements tfvisConstants{
	...
	void setLoopInitTarget(String var,boolean tmp){m_LoopInitTargetName=var;m_LoopInitTargetIsTmp=tmp;}
	String getLoopInitTargetName(){return m_LoopInitTargetName;}
	boolean isLoopInitTargetTmp(){return m_LoopInitTargetIsTmp;}
	...
	void setMethodID(int id){m_MethodID=id;}
	int getMethodID(){return m_MethodID;}
	
	ArrayList<Integer> getEventList(){return m_Event;}
	
	void setIndent(int in){m_Indent=in;}
	int getIndent(){return m_Indent;}
	String getIndentText(){return Tools.getIndetStr(m_Indent);}
	...
	boolean hasUpdateEvent(){
		for(int indexID:m_Event){
			...
			if(indexID >= Ev_UpdateRangeL && indexID <= Ev_UpdateRangeH){return true;}
		}
	 ...
	 * @param text 変数名
	 */
	void setTarget(String text){m_Target=text;m_TargetList.add(m_Target);}
	String getTarget(){return m_Target; }
	ArrayList<String> getTargetList(){return m_TargetList;}
	 ...
	 * @param text 追加するコード
	 */
	void addCode(String text){m_Code+=text;}	
	
	/**
	 * イベントを登録
	 * @param id イベントのID
	 */
	void addEvent(int id){m_Event.add(id);}
	
	void addEventTop(int id){m_Event.add(0, id);}
	...
	boolean deleteUse(String text){
		...
		for(int i=0;i<m_Uses.size();i++){
			if(m_Uses.get(i).equals(text) ){ m_Uses.remove(i); return true;}
		}
		return false;
	}	
	String getUse(int num){return m_Uses.get(num); }
	int getUseNum(){return m_Uses.size();}
	
	
	/**
	 * 行データを出力//
	 * イベント番号#更新する変数#参照する変数#行別コード#
	 */
	void fileOutput(PrintWriter fout){
		...
		if(m_Event.size()==0){fout.print(Ev_Other+",");}
