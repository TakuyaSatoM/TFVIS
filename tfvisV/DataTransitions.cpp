...
bool DtDiagram::drawTransitionsDiagram(DTCom* dt){
	drawCode(dt);
   drawTable(dt);
...
bool DtDiagram::drawInstanceDiagram(DTCom* dt){
	...
	//ウィンドウ部
   {
		...
		sprintf(DWordBuffer(),"class %s/%s",instance->m_Type.c_str(), instance->m_Target.c_str());
		DWordDrawText(G()->m_CommonFont_S  ,DWordBuffer());	
		DWordDrawEnd();
...
bool DtDiagram::draw(DTCom* dt)
{
   ...
   if(dt->m_MethodExe != NULL){
		result = drawTransitionsDiagram(dt);
   }else{
	   result = drawInstanceDiagram(dt);
   }
